 ...
 * @author brc
 */
public class labor30Graph extends JPanel{
    ...
    try {
      ...
      ResultSet rs = stmt.executeQuery( "SELECT Serv_Date,SUM(Serv_Hours) FROM service where (julianday(current_date)-julianday(Serv_Date)) <= 30 group by Serv_Date;" );
      
      while ( rs.next()) {
          
           String day = rs.getString("Serv_Date");
           String delims = "[-]+";
           String[] dP = day.split(delims);
           double dCount = rs.getDouble("SUM(Serv_Hours)");
           hours.set((Integer.parseInt(dP[2])), dCount);
      ...
      System.err.println( e.getClass().getName() + ": " + e.getMessage() );
        ...
        MAX_SCORE = ((int)(max));
        labor30Graph mainPanel = new labor30Graph(hours);
        Y_HATCH_CNT = MAX_SCORE;
    }
    
     public labor30Graph(List<Double> hours){
        this.hours = hours;
